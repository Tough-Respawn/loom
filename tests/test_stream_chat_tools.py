# Contrats observables du streaming et des chemins séquentiel/parallèle.
...
def test_reponse_vide_relances_puis_done():
    # Deux relances vides précèdent l'arrêt par défaut.
    client, fake = make_client([turn_text(""), turn_text(""), turn_text("")])
    events, done = run(client)
    assert done["reason"] == "empty_response"
    assert len(fake.calls) == 3
    # Les relances appartiennent à la voix du harnais, pas à un faux résultat d'outil.
...
def test_continuation_length():
    ...
    # Le second appel conserve le texte assistant avant le nudge.
...
def test_payload_write_file_specialise():
    # Préserver les détails spécialisés de write_file dans le payload unifié.
...
def test_parallele_distant():
    ...
    # Tous les appels doivent être annoncés avant le premier résultat parallèle.
    ks = kinds(events)
    assert ks.index("tool_result") > len(ks) - 1 - ks[::-1].index("tool_call")

    # Les résultats gardent l'ordre des appels et le même payload que le séquentiel.
...
def test_harness_est_une_3e_voix_marquee_loom():
    # Une intervention Loom doit être identifiable dans le prompt comme dans l'UI.
    client, fake = make_client([turn_text(""), turn_text("ok.")])
    events, done = run(client)
    harness = only(events, "harness")
    assert harness and harness[0]["kind"] == "réponse vide"
    # Le tag minimal suffit car sa signification vit dans le prompt système stable.
    injected = [
        m
...
def test_system_prompt_explique_le_tag_loom():
    # Définir le tag dans le préfixe stable évite de repayer son explication à chaque relance.
...
def test_note_au_stop_naturel_relance():
    # Une note arrivée après le drain initial doit relancer au stop naturel.
    calls = {"n": 0}
...
def test_repeat_stop_meme_appel_repete():
    # Trois répétitions identiques déclenchent l'arrêt par défaut.
...
def test_boucle_degeneree_texte_repete():
    ...
    # Deux nudges précèdent l'arrêt pour boucle dégénérée.
...
def test_strong_desactive_repeat_stop():
    # Un modèle distant fort continue jusqu'à son arrêt naturel.
