...
def test_agents_defaults_empty_when_absent(tmp_path):
    cfg_path = _write(tmp_path, "loom.config.toml", BASE)
    cfg = load_config(cfg_path)
    assert cfg.agents == []
    assert cfg.default_pipeline == []


def test_agents_read_from_file(tmp_path):
    text = (
        BASE
        + '\n[[agents]]\nid = "planner"\nrole = "plan"\nmodel = "m1"\n'
        + 'system_prompt = "Tu planifies."\nskills = ["dagster"]\nmax_tokens = 512\n'
        + '\n[[agents]]\nid = "coder"\nrole = "code"\nmodel = "m2"\n'
        + 'system_prompt = "Tu codes."\n'
        + '\n[chat]\ndefault_pipeline = ["planner", "coder"]\n'
    )
    cfg_path = _write(tmp_path, "loom.config.toml", text)
    cfg = load_config(cfg_path)
    assert [a.id for a in cfg.agents] == ["planner", "coder"]
    planner = cfg.agent_by_id("planner")
    assert planner.role == "plan"
    assert planner.model == "m1"
    assert planner.system_prompt == "Tu planifies."
    assert planner.skills == ["dagster"]
    assert planner.max_tokens == 512
    coder = cfg.agent_by_id("coder")
    assert coder.skills == []
    assert coder.max_tokens is None
    assert cfg.default_pipeline == ["planner", "coder"]


def test_agent_by_id_unknown_returns_none(tmp_path):
    cfg_path = _write(tmp_path, "loom.config.toml", BASE)
    cfg = load_config(cfg_path)
    assert cfg.agent_by_id("zzz") is None
